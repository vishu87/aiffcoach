...
class ApprovalController extends BaseController {
    ...
    public function pendingLicenses(){
        $sql = CoachLicense::listing()->join('users','users.coach_id','=','coach_licenses.coach_id')->where('users.official_types','LIKE','%'.Auth::user()->manage_official_type.'%')->where('coach_licenses.status','!=',1)->where('coaches.status',1);
        if(Input::get("registration_id") != ''){
          $sql = $sql->where('coaches.registration_id','LIKE','%'.Input::get('registration_id').'%');
        }
        if(Input::get("official_name") != ''){
    ...
    public function pendingEmploymentDetails(){
        $sql = EmploymentDetails::select('employment_details.*','coaches.full_name')->leftJoin('coaches','employment_details.coach_id','=','coaches.id')->join('users','users.coach_id','=','employment_details.coach_id')->where('users.official_types','LIKE','%'.Auth::user()->manage_official_type.'%')->where('employment_details.status','!=',1)->where('coaches.status',1);
        if(Input::get("registration_id") != ''){
          $sql = $sql->where('coaches.registration_id','LIKE','%'.Input::get('registration_id').'%');
        }
        if(Input::get("official_name") != ''){
